...
- `OPERATORS.md` — the seven-operator algebra (split, merge, move, hoist, demote, migrate, glue) plus the optional `delete` and the required `finalize_proposal`. Defines validation rules: ≤15 glue tokens per op, ≤60 total, ≤50% migrate change-pct, no forbidden tokens.
