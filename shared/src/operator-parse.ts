import type { Operator, Position } from "./alfred-types.js";
...
/**
 * Parse an array of raw operator objects into typed Operator[].
 * Accepts two shapes per element:
 *   - { name: string, input: Record<string, unknown> }  (Anthropic tool-use shape)
 *   - { kind: string, ...fields }                       (direct operator shape)
 */
export function parseOperators(input: unknown): Operator[] {
  if (!Array.isArray(input)) {
    throw new Error(`parseOperators: expected an array, got ${typeof input}`);
  }
  return input.map((item, i) => {
    if (!item || typeof item !== "object") {
      throw new Error(`parseOperators[${i}]: expected an object`);
    }
    const obj = item as Record<string, unknown>;

    // Shape 1: { name, input }
    if (typeof obj.name === "string" && obj.input && typeof obj.input === "object") {
      return parseOperator(obj.name, obj.input as Record<string, unknown>);
    }

    // Shape 2: { kind, ...fields }
    if (typeof obj.kind === "string") {
      const { kind, ...fields } = obj;
      return parseOperator(kind as string, fields as Record<string, unknown>);
    }

    throw new Error(`parseOperators[${i}]: object must have either {name, input} or {kind, ...fields}`);
  });
}
