...
export async function resolveBackend(
  ...
  try {
    ...
    // Email provisioning removed — Alfred doesn't use email routing.
...
export async function drainWork(env: Env): Promise<DrainResult[]> {
  ...
  for (let i = 0; i < MAX_DRAIN; i++) {
    ...
    // MicroVM/Container path removed — Alfred uses isolate-only.
    // If backend resolves to microvm, log and skip.
    console.warn(
      `[webhook] skipping microvm session=${sessionId} — Alfred is isolate-only`,
    );
    continue;
  }
...
export async function handleWebhook(
  ...
  switch (evType) {
    ...
    case "session.status_idled": {
      ...
      //
      // Isolate sessions persist via DO SQLite — no snapshot needed.
      console.log(`[webhook] action=session-terminated ${eventSummary}`);
      break;
    }
    default: {
